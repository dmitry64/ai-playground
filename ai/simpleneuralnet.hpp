...
struct NeuralNetParams {
    ...
    NeuralNetParams (const NeuralNetParams & params) : _eta(params._eta), _alpha(params._alpha) { }
};
...
public:
    ...
    Neuron(const Neuron & copy, const NeuralNetParams & params) : _params(params) {
        _outValue = copy._outValue;
        _gradient = copy._gradient;
        _myIndex = copy._myIndex;
        _outputWeights = copy._outputWeights;
    }
...
public:
    ...
    NeuralNet(const Topology & topology, NeuralNetParams & params) : _params(params), _topology(topology)
    {
    ...
    NeuralNet(const NeuralNet & copy) : _params(NeuralNetParams(copy._params)), _topology(copy._topology) {
        assert(copy._topology.size());
        unsigned int numLayers = static_cast<unsigned int>(copy._topology.size());
        for(unsigned int layerNum = 0; layerNum < numLayers; ++layerNum) {
            const Layer & oldlayer = copy._layers[layerNum];
            Layer newLayer;
            for(unsigned int num = 0; num < oldlayer.size(); ++num) {
                const NeuronType & neuron = oldlayer.at(num);
                newLayer.push_back(NeuronType(neuron,_params));
            }
        ...
        _error = copy._error;
        _recentAverageSmoothingFactor = copy._recentAverageSmoothingFactor;
        _recentAverageError = copy._recentAverageError;
    }
